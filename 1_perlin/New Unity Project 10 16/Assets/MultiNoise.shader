	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f_customrendertexture i) : SV_Target
			{
				...
				float noise = (Noise(i.globalTexcoord * 4.0 + _Time.y * 1.05))
							+ (Noise(i.globalTexcoord * 16.0 + _Time.y * 1.20)) * 0.5
							+ (Noise(i.globalTexcoord * 32.0 + _Time.y * 1.80)) * 0.25;

				noise = noise / (1.0 + 0.5 + 0.25) * 0.5 + 0.5; //正規化
				noise = sin(500 * noise); //縞模様
				return float4 (noise + 255, noise, noise, 1); //正規化
